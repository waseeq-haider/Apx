...
export default function Hero() {
    ...
    return (
        <section className="pt-12 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative">
            ...
            <div className="reveal active bg-white/80 backdrop-blur-xl rounded-[3rem] p-8 md:p-16 shadow-sm border border-purple-100 relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-3xl">
                    ...
                    <h1 className="reveal active delay-200 text-5xl md:text-7xl font-extrabold text-slate-900 leading-[1.1] mb-6">
                        Home Experts at <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">Your Door.</span>
                    </h1>
                    <p className="reveal active delay-300 text-xl text-slate-500 mb-10 max-w-xl leading-relaxed">
                    ...
                    <div className="reveal active delay-500 flex flex-wrap gap-4">
                        <Link to="/quote" className="bg-slate-900 hover:bg-slate-800 text-white px-8 py-4 rounded-full font-semibold text-lg flex items-center transition-transform hover:-translate-y-1 shadow-xl shadow-slate-900/10">
                            Request a Quote <ArrowRight className="ml-2 w-5 h-5" />
                        </Link>
                        <button onClick={() => navigate('/chat')} className="bg-white border-2 border-slate-200 hover:border-slate-300 text-slate-700 px-8 py-4 rounded-full font-semibold text-lg flex items-center transition-colors">
                            <Phone className="mr-2 w-5 h-5" /> Request a Call
                        </button>
                        <button onClick={() => navigate('/chat')} className="bg-purple-50 hover:bg-purple-100 text-purple-700 px-6 py-4 rounded-full font-semibold text-lg flex items-center transition-colors">
                            <Camera className="mr-2 w-5 h-5" /> Send Photos
                        </button>
                    </div>

                    {/* Search Bar Visual */}
                    <div className="reveal active delay-700 mt-12 bg-white shadow-xl shadow-purple-100/50 rounded-2xl p-2 flex items-center max-w-2xl border border-slate-100">
                        <div className="pl-4 pr-2 text-slate-400">
                            <MapPin className="w-5 h-5" />
                        </div>
                        <input type="text" placeholder="Detroit, MI" className="w-32 bg-transparent border-none focus:ring-0 text-slate-800 placeholder-slate-400 font-medium outline-none" />
                        <div className="h-6 w-px bg-slate-200 mx-2"></div>
                        <input type="text" placeholder="What services are you looking for?" className="flex-1 bg-transparent border-none focus:ring-0 text-slate-800 placeholder-slate-400 outline-none" />
